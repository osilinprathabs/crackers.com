...
{
    /**
     * ERP invoice / billing tables are optional; loan-only installs may not have them.
     */
    public function hasSalesInvoiceTables(): bool
    {
        return Schema::hasTable('sales_invoices');
    }

    public function hasPurchaseInvoiceTables(): bool
    {
        return Schema::hasTable('purchase_invoices');
    }

    protected function emptyAgingSummary(): array
    {
        return [
            'current' => 0,
            '1_30_days' => 0,
    ...
    public function getInvoiceAging($filters = [])
    {
        ...
        if (! Schema::hasTable('sales_invoices') || ! Schema::hasTable('users')) {
            return [
                'aging_summary' => $this->emptyAgingSummary(),
                'customers' => [],
                'as_of_date' => $asOfDate,
                'erp_tables_missing' => true,
            ];
        }
    ...
    public function getBillAging($filters = [])
    {
        ...
        if (! Schema::hasTable('purchase_invoices') || ! Schema::hasTable('users')) {
            return [
                'aging_summary' => $this->emptyAgingSummary(),
                'vendors' => [],
                'as_of_date' => $asOfDate,
    ...
    public function getTaxSummary($filters = [])
    {
        ...
        $salesTaxReady = Schema::hasTable('sales_invoice_item_taxes')
            && Schema::hasTable('sales_invoice_items')
            && Schema::hasTable('sales_invoices');
        $purchaseTaxReady = Schema::hasTable('purchase_invoice_item_taxes')
            && Schema::hasTable('purchase_invoice_items')
            && Schema::hasTable('purchase_invoices');

        if (! $salesTaxReady && ! $purchaseTaxReady) {
            return [
                'tax_collected' => [
                    'items' => collect(),
                    'total' => 0,
        ...
        $taxPaid = $purchaseTaxReady
            ? DB::table('purchase_invoice_item_taxes')
    ...
    public function getCustomerBalanceSummary($filters = [])
    {
        ...
        $hasSalesReturns = Schema::hasTable('sales_invoice_returns');
        ...
        foreach ($customers as $customer) {
            ...
            $returns = $hasSalesReturns
                ? DB::table('sales_invoice_returns')
                    ->where('customer_id', $customer->id)
    ...
    public function getCustomerDetail($customerId, $filters = [])
    {
        ...
        if (Schema::hasTable('sales_invoice_returns')) {
            ...
            if ($startDate) {
                $returnsQuery->where('return_date', '>=', $startDate);
            }
            if ($endDate) {
                $returnsQuery->where('return_date', '<=', $endDate);
            }
        ...
        if (Schema::hasTable('credit_notes')) {
            ...
            if ($startDate) {
                $creditNotesQuery->where('credit_note_date', '>=', $startDate);
            }
            if ($endDate) {
                $creditNotesQuery->where('credit_note_date', '<=', $endDate);
            }
        ...
        if (Schema::hasTable('customer_payments')) {
            ...
            if ($startDate) {
                $paymentsQuery->where('customer_payments.payment_date', '>=', $startDate);
            }
            if ($endDate) {
                $paymentsQuery->where('customer_payments.payment_date', '<=', $endDate);
            }
            $payments = $paymentsQuery->orderBy('customer_payments.payment_date', 'desc')->get();
        } else {
            $payments = collect();
        }
    ...
    public function getVendorDetail($vendorId, $filters = [])
    {
        ...
        if (Schema::hasTable('debit_notes')) {
            ...
            if ($startDate) {
                $debitNotesQuery->where('debit_note_date', '>=', $startDate);
            }
            if ($endDate) {
                $debitNotesQuery->where('debit_note_date', '<=', $endDate);
            }
        ...
        if (Schema::hasTable('vendor_payments')) {
            if (Schema::hasTable('bank_accounts')) {
                $paymentsQuery = DB::table('vendor_payments')
                    ...
                    ->select(
                        ...
                        'vendor_payments.payment_amount as amount',
                        'vendor_payments.reference_number',
            ...
            if ($startDate) {
                $paymentsQuery->where('vendor_payments.payment_date', '>=', $startDate);
            }
            if ($endDate) {
                $paymentsQuery->where('vendor_payments.payment_date', '<=', $endDate);
            }
            $payments = $paymentsQuery->orderBy('vendor_payments.payment_date', 'desc')->get();
        } else {
            $payments = collect();
        }
