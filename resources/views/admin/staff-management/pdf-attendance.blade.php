<div style="width: 100%;">
    <h3 style="text-align: center; margin-bottom: 20px;">Staff Attendance Report - {{ date('F Y', mktime(0, 0, 0, $month, 1, $year)) }}</h3>
    
    <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
        <thead>
            <tr style="background-color: #f8f9fa;">
                <th style="border: 1px solid #dee2e6; padding: 5px;">Staff Member</th>
        ...
        <tbody>
            @foreach($staffs as $s)
                ...
                @endphp
                <tr>
                    <td style="border: 1px solid #dee2e6; padding: 5px; font-weight: bold;">{{ $s->name }}</td>
                    @for($d=1; $d<=$daysInMonth; $d++)
                        @php
