...
@if ($mode === 'csv')
<table>
  <tr><th colspan="2">{{ __('Bill aging') }} — {{ __('As of') }} {{ $asOf }}</th></tr>
  ...
  <tr><th colspan="7">{{ __('By vendor') }}</th></tr>
  <tr>
    <th>{{ __('Vendor') }}</th>
    <th>{{ __('Current') }}</th>
    <th>1–30</th>
...
<body>
  ...
  <table>
    <thead><tr><th>{{ __('Bucket') }}</th><th class="num">{{ __('Amount') }} (₹)</th></tr></thead>
    <tbody>
  ...
  <h3 style="font-size: 13px;">{{ __('By vendor') }}</h3>
  <table>
    <thead>
      <tr>
        <th>{{ __('Vendor') }}</th>
