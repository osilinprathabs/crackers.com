...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div>
      <h4 class="mb-1">{{ __('Expenses') }}</h4>
      <p class="text-muted mb-0">{{ __('Record costs; approve then post to the general ledger.') }}</p>
    </div>
    <div class="d-flex align-items-center gap-2 flex-wrap">
      ...
      @endphp
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addExpenseDraftModal">
        <i class="ri-add-line me-1"></i> {{ __('New expense') }}
      </button>
      <button type="button" class="btn btn-outline-secondary ms-2" data-bs-toggle="modal" data-bs-target="#addExpenseCategoryModal">
        <i class="ri-bookmark-2-line me-1 text-danger"></i> {{ __('Add category') }}
      </button>
    ...
    <div class="card-body p-3">
      <form method="get" class="row g-2 align-items-center">
        ...
        <div class="col-md-2 d-flex gap-1">
          ...
          <a href="{{ route('account.expenses.index') }}" class="btn btn-outline-secondary px-2" title="{{ __('Reset') }}"><i class="ri-refresh-line"></i></a>
        </div>
      </form>
    </div>
  </div>
      ...
      <table class="table table-hover mb-0">
        ...
        <tbody>
          @forelse ($expenses as $e)
            <tr>
              <td><a href="{{ route('account.expenses.show', $e) }}">{{ $e->expense_number }}</a></td>
