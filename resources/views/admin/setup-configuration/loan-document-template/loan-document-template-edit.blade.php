...
@section('title', 'Edit Loan Document Template')
  ...
  <div class="col-12">
    <div class="card mb-6">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Edit Document Template</h5>
      ...
      <div class="card-body">
        <form action="{{ route('loan-document-templates.update', ['loan_document_template' => $template->id]) }}" method="POST" id="loanTemplateForm">
          @csrf
          @method('PUT')
          ...
          <div class="mb-4">
            ...
            >
              ...
              <option value="loan_agreement" {{ old('type', $template->type) == 'loan_agreement' || str_starts_with($template->type, 'loan_agreement_') ? 'selected' : '' }}>Loan Agreement</option>
              <option value="loan_sanction_letter" {{ old('type', $template->type) == 'loan_sanction_letter' ? 'selected' : '' }}>Loan Sanction Letter</option>

              <option value="disbursement_letter" {{ old('type', $template->type) == 'disbursement_letter' ? 'selected' : '' }}>Disbursement Letter</option>
              <option value="statement" {{ old('type', $template->type) == 'statement' ? 'selected' : '' }}>Loan Statement</option>
              <option value="noc" {{ old('type', $template->type) == 'noc' ? 'selected' : '' }}>NOC (No Objection Certificate)</option>
              <option value="loan_closure_certificate" {{ old('type', $template->type) == 'loan_closure_certificate' ? 'selected' : '' }}>Loan Closure Certificate</option>
              <option value="foreclosure_letter" {{ old('type', $template->type) == 'foreclosure_letter' ? 'selected' : '' }}>Foreclosure Letter</option>
              <option value="demand_letter" {{ old('type', $template->type) == 'demand_letter' ? 'selected' : '' }}>Demand Letter (Overdue Notice)</option>
              <option value="reminder_notice" {{ old('type', $template->type) == 'reminder_notice' ? 'selected' : '' }}>Reminder Notice (Upcoming / Overdue Reminder)</option>
              <option value="other" {{ old('type', $template->type) && !in_array(old('type', $template->type), ['loan_sanction_letter', 'disbursement_letter', 'statement', 'noc', 'loan_closure_certificate', 'foreclosure_letter', 'demand_letter', 'reminder_notice']) ? 'selected' : '' }}>Other</option>
            </select>
            @error('type')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <div id="loanProductWrapper" class="mt-3" style="display: {{ (old('type', $template->type) == 'loan_agreement' || str_starts_with($template->type, 'loan_agreement_')) ? 'block' : 'none' }};">
              ...
              <select name="loan_product_id" id="loan_product_id" class="form-select @error('loan_product_id') is-invalid @enderror">
                ...
                @foreach($loanProducts as $product)
                  @php
                    $computedType = 'loan_agreement_' . strtolower(str_replace(' ', '_', $product->loan_name));
                    $isSelected = old('loan_product_id') == $product->id || $template->type == $computedType;
                  @endphp
                  <option value="{{ $product->id }}" {{ $isSelected ? 'selected' : '' }}>{{ $product->loan_name }}</option>
                @endforeach
              </select>
            ...
            <div id="customTypeWrapper" class="mt-3" style="display: none;">
              ...
              <input type="text" id="customType" class="form-control" placeholder="Enter custom document type" value="{{ old('type', $template->type) && !in_array(old('type', $template->type), ['loan_agreement', 'loan_sanction_letter', 'disbursement_letter', 'statement', 'noc', 'loan_closure_certificate', 'foreclosure_letter', 'demand_letter', 'reminder_notice']) ? old('type', $template->type) : '' }}">
          ...
          <div class="d-flex gap-3">
            <button type="submit" class="btn btn-primary">
              <i class="icon-base ri ri-save-line me-1"></i> Update Template
            </button>
            <a href="{{ route('loan-document-templates.index') }}" class="btn btn-outline-secondary">
