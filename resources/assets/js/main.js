...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Preloader transition when switching menu pages
  const pageLoader = document.querySelector('.page-loader');
  if (pageLoader) {
    // Select all links in the sidebar menu (inner links and brand logo links)
    const sidebarLinks = document.querySelectorAll('#layout-menu a[href], .app-brand-link[href], .app-brand a[href]');
    sidebarLinks.forEach(link => {
      link.addEventListener('click', function (e) {
        const href = this.getAttribute('href');
        // Do not trigger for blank links, JavaScript calls, page hashes, or new tab links
        if (
          !href ||
          href.trim() === '' ||
          href.startsWith('#') ||
        ...
        // Save transition flag in sessionStorage to keep preloader in offset blurred mode on the next page
        try {
          sessionStorage.setItem('menuTransition', 'true');
        } catch (err) {}

        // Show the blurred preloader (excluding the sidebar) during transition
...
// Safety fallback: Force hide loader after 3 seconds just in case an external asset is hanging the 'load' event
...
(function () {
  ...
  // Global Password Toggle for data-action="toggle-password"
...
function loadSearchData() {
  ...
  const searchJson = (typeof $ !== 'undefined' && $('#layout-menu').hasClass('menu-horizontal')) ? 'search-horizontal.json' : 'search-vertical.json';
  ...
  fetch(assetsPath + 'json/' + searchJson)
    ...
    .catch(error => {
      ...
      // Fallback: build a deterministic search index from visible menu links.
      data = buildMenuSearchData();
      initializeAutocomplete();
    });
}

function buildMenuSearchData() {
  const menuLinks = Array.from(document.querySelectorAll('#layout-menu .menu-link[href]'));
  const navigation = { quick: [] };

  menuLinks.forEach(link => {
    const href = (link.getAttribute('href') || '').trim();
    const name = (link.textContent || '').replace(/\s+/g, ' ').trim();
    if (!href || !name || href === 'javascript:void(0);' || href.startsWith('#')) return;
    navigation.quick.push({
      name,
      url: href.startsWith('http') ? href : href.replace(/^\//, ''),
      icon: 'ri-search-line'
    });
  });

  return { suggestions: { Quick: navigation.quick.slice(0, 8) }, navigation };
}
...
$(document).on('click', '.search-toggler', function () {
  ...
  if (searchButton) {
    ...
    // Fallback: try to find any button inside the autocomplete container
    const autocompleteEl = document.getElementById('autocomplete');
    if (autocompleteEl) {
      const btn = autocompleteEl.querySelector('button');
      if (btn) btn.click();
    }
  }
});
...
function initializeAutocomplete() {
  ...
  const autocompleteFn = window.autocomplete || (window['@algolia/autocomplete-js'] && window['@algolia/autocomplete-js'].autocomplete);
  if (!autocompleteFn) {
    // Fallback to a simple onChange/onKeyUp global filter search if autocomplete is unavailable.
    initializeFallbackGlobalSearch(searchElement);
    // Keep retrying once in background in case library arrives later.
  ...
  return autocompleteFn({
    ...SearchConfig,
    openOnFocus: true,
    detachedMediaQuery: 'all', // Ensure it works on all screen sizes
          ...
          ([section, items]) => html`
                  <div class="col-md-6 suggestion-section">
                    ...
                    <div class="suggestion-items">
                      ${items.map(
            item => html`
                  <a href="${(window.baseUrl || '/')}${item.url}" class="suggestion-item d-flex align-items-center">
      ...
      if (data.navigation) {
        ...
        const navigationSources = Object.keys(data.navigation)
          ...
          .map(section => ({
            ...
            getItemUrl({ item }) {
              return (window.baseUrl || '/') + item.url;
            },
            templates: {
              header({ items, html }) {
              ...
              item({ item, html }) {
                return html`
                  <a href="${(window.baseUrl || '/')}${item.url}" class="d-flex justify-content-between align-items-center">
        ...
        if (data.navigation.files) {
          sources.push({
            ...
            templates: {
              ...
              item({ item, html }) {
                return html`
                  <a href="${(window.baseUrl || '/')}${item.url}" class="d-flex align-items-center position-relative px-4 py-2">
                    <div class="file-preview me-2">
                      <img src="${(window.assetsPath || '/assets/')}${item.src}" alt="${item.name}" class="rounded" width="42" />
                    </div>
        ...
        if (data.navigation.members) {
          sources.push({
            ...
            templates: {
              ...
              item({ item, html }) {
                return html`
                  <a href="${(window.baseUrl || '/')}${item.url}" class="d-flex align-items-center py-2 px-4">
                    <div class="avatar me-2">
                      <img src="${(window.assetsPath || '/assets/')}${item.src}" alt="${item.name}" class="rounded-circle" width="32" />
                    </div>
                    <div class="flex-grow-1">
                      <h6 class="mb-0">${item.name}</h6>
                      <small class="text-body-secondary">${item.subtitle}</small>
...
function initializeFallbackGlobalSearch(searchElement) {
  if (!searchElement || searchElement.dataset.fallbackSearchInit === 'true') return;
  searchElement.dataset.fallbackSearchInit = 'true';

  searchElement.innerHTML = `
    <input
      type="search"
      id="globalSearchFallback"
  ...
  const fallbackInput = searchElement.querySelector('#globalSearchFallback');
  ...
  fallbackInput.addEventListener('keyup', handleSearch);
  fallbackInput.addEventListener('change', handleSearch);
}
...
document.addEventListener('keydown', event => {
  if ((event.ctrlKey || event.metaKey) && event.key === 'k') {
    ...
    if (searchButton) {
      ...
      // Fallback if the library hasn't initialized the button yet
      loadSearchData();
    }
  }
});

// Search Toggler click handler
document.addEventListener('click', event => {
  if (event.target.closest('.search-toggler')) {
    event.preventDefault();
    const searchButton = document.querySelector('.aa-DetachedSearchButton');
    if (searchButton) {
      searchButton.click();
    } else {
      // If not initialized, try to load and then click (might need a small delay)
      loadSearchData();
      setTimeout(() => {
